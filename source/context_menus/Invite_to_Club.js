const { PermissionFlagsBits, InteractionContextType, ActionRowBuilder, StringSelectMenuBuilder, userMention, ButtonBuilder, ButtonStyle } = require('discord.js');
...
module.exports = new UserContextMenuWrapper(mainId, PermissionFlagsBits.SendMessages, [InteractionContextType.Guild], 3000,
	(interaction) => {
		if (interaction.targetUser.bot) {
			interaction.reply({ content: "If you'd like at add a bot to your club, please speak with a moderator.", ephemeral: true });
			return;
		}
		...
		if (clubOptions.length < 1) {
			interaction.reply({ content: "You can invite server members to clubs you are a member of that are still recruiting. There don't appear to be any clubs in that list.", ephemeral: true });
			return;
		}
		...
		interaction.reply({
			...
			collector.on("collect", collectedInteraction => {
				...
				if (club.hostId === interaction.targetId || club.userIds.includes(interaction.targetId)) {
					collectedInteraction.reply({ content: `${userMention(interaction.targetId)} is already a member of ${club.title}.`, ephemeral: true });
					return;
				}

				if (!club.isRecruiting()) {
					collectedInteraction.reply({ content: `Your invite to ${club.title} was not sent. The club is full!`, ephemeral: true });
					return;
				}
				...
				collectedInteraction.reply({ content: `Details about and an invite to ${club.title} have been sent to ${userMention(interaction.targetId)}.`, ephemeral: true });
			})
